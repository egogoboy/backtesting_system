...
class Signal {
  public:
    static Signal make_market(const Instrument &instrument, Direction direction, double stop_loss,
                              double take_profit) {
    ...
    double get_stop_loss_price() const {
        return stop_loss_price_;
    }

    double get_take_profit_price() const {
        return take_profit_price_;
    }

  private:
    Signal(const Instrument &instrument, Direction direction, OrderType type,
           std::optional<double> entry_price, double stop_loss, double take_profit)
