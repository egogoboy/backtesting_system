...
TEST(Order, Types) {
    Order order = Order::make_market(GLOBAL_EURUSD_INSTRUMENT, Direction::LONG, 1);
    EXPECT_EQ(order.get_type(), OrderType::MARKET);

    order = Order::make_limit(GLOBAL_EURUSD_INSTRUMENT, Direction::LONG, 1, 1);
    EXPECT_EQ(order.get_type(), OrderType::LIMIT);

    order = Order::make_stop(GLOBAL_EURUSD_INSTRUMENT, Direction::LONG, 1, 1);
...
TEST(Order, Id) {
    Order first_order = Order::make_market(GLOBAL_EURUSD_INSTRUMENT, Direction::LONG, 1);
    EXPECT_EQ(first_order.get_id(), 1);

    Order second_order = Order::make_market(GLOBAL_EURUSD_INSTRUMENT, Direction::LONG, 1);
